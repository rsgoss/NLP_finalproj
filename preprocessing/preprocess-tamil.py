...
# files to process
files = ['tamil_train', 'tamil_dev', 'tamil_test']
...
punctuation2 = ['RT', '_', '.', '…', '...', '!', '#', '__', '___', '____', '_____', '..', '....', '.....', '.…', '.....',
               ...
               '>', '<', '”', '“', '"', '.......................', '►',
                ...
                'ാ', 'അ', ' ൂ', 'േ', 'െ', 'ൊ', 'ഈ', ' ൈ', 'െ', 'ൊ',
                'అ', 'భ', 'ఆ', 'భా', 'ఇ', 'భి', 'ఈ', 'భీ', 'ఉ', 'భు', 'ఊ', 'భూ', 'ఋ', 'భృ', 'ౠ', 'భౄ', 'ఌ', 'భౢ',
                'ౡ', 'భౣ', 'ఎ', 'భె', 'ఏ', 'భే', 'ఐ', 'భై', 'ఒ', 'భొ', 'ఓ', 'భో', 'ఔ', 'భౌ', 'అం', 'భం', 'అః', 'భః']
...
def cleaning_punctuation(sentence):
    ...
    tokenlist = re.compile(r'\w+|[^\x00-\x7F]+').findall(sentence)
    # nonUTF = re.compile('[^\x00-\x7F]+').findall(sentence)
    # the following lines to remove nonUTF
    ## Remove non UTF: |[^\w\s]+
    # nonUTF = (' '.join(nonUTF))
    # nonUTF = re.compile(r'\w+|[^\w\s]+').findall(nonUTF)
    # print(tokenlist)
    ...
    for token in tokenlist:
        ...
        # elif any(punct in token for punct in nonUTF):
        #     # print(token)
        #     continue
        else:
...
for file in files:
    ...
    datapath = filepath + '/' + 'data' + '/' + 'tamil-english' + '/' + file
    outpath = filepath + '/' + 'data' + '/' + 'tamil-english' + '/' + 'processed' + '/' + file

    data = pd.read_csv(datapath + '.tsv', sep="\t", engine='python', quoting=csv.QUOTE_NONE)

    data = data[~data.category.str.contains("unknown_state")]
    data = data[~data.category.str.contains("not-Tamil")]
    ...
    with alive_bar(len(data.index)) as bar:
        for i in data.index:
            ...
            bar()
    data2.to_csv(outpath + ".csv", ',', encoding="utf-8")
